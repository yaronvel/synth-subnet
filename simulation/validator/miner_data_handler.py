    ...
    def set_reward_details(reward_details: [], start_time: str):
        ...
        with engine.begin() as connection:
            try:
                ...
                connection.rollback()
                bt.logging.info(
                    f"in set_reward_details (got an exception): {e}"
                )
        ...
        try:
            ...
            with engine.connect() as connection:
                query = (
                    ...
                    .where(
                        (
                            miner_predictions.c.start_time
                            + text("INTERVAL '1 second'")
                            * miner_predictions.c.time_length
                        )
                        < validation_time,
            ...
            bt.logging.info(
                "in get_values, predictions fetched for miner_uid: "
                + str(miner_uid)
            )
            ...
            bt.logging.info(
                "in get_values, predictions length:" + str(len(result[0]))
            )
